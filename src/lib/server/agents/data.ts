...
import { DataAgentInputSchema, GeneratedProofSchema } from './contracts';
import type { ProofInsight, GeneratedProof } from './contracts';
import { openaiChat } from '../openai';
import { getRecentEvents, Event } from '../database';
import { withMetrics } from '../metrics';

export type { ProofInsight, GeneratedProof };

async function _generateProofFromEvents(
  storyId: string,
  persona: string,
...
export async function generateProofFromEvents(
  ...
  DataAgentInputSchema.parse({ storyId, persona, minutesBack });
  const result = await withMetrics('data', 'generateProof', () =>
    _generateProofFromEvents(storyId, persona, minutesBack)
  );
  return GeneratedProofSchema.parse(result);
}
...
function analyzeEvents(events: Event[]): ProofInsight[] {
  ...
  if (dwellEvents.length > 0) {
    const avgDwellTime =
      dwellEvents.reduce(
        (sum, e) => sum + (Number((e.meta as Record<string, unknown>)?.duration) || 0),
        0
      ) / dwellEvents.length;
  ...
  if (scrollDepthEvents.length > 0) {
    const maxScrollDepth = Math.max(
      ...scrollDepthEvents.map((e) => Number((e.meta as Record<string, unknown>)?.depth) || 0)
    );
...
async function generateQuotesFromInsights(
  ...
  Array<{ text: string; role: string; source: 'generated' | 'real'; credibility: number }>
> {
...
function getDefaultQuotes(
  persona: string
): Array<{ text: string; role: string; source: 'generated'; credibility: number }> {
  const quotesByPersona: Record<string, Array<{ text: string; role: string }>> = {
    athlete: [
    ...
    family: [
      ...
      {
        text: "Finally, a bottle I trust with my kids' health.",
        role: 'Pediatric Nurse',
      },
    ],
  };

  const quotes = quotesByPersona[persona] || quotesByPersona.commuter;
...
export async function updateProofSection(
  ...
  return withMetrics('data', 'updateProof', async () => {
    const freshProof = await _generateProofFromEvents(storyId, persona);
    ...
    return { ...currentSection, quotes: bestQuotes };
  });
}

export function generateSocialProofMetrics(
  insights: ProofInsight[]
...
export function analyzeConversionFunnel(events: Event[]): {
  ...
  const steps = [
    { name: 'Page Views', count: views },
    {
      name: 'Engaged (Hover)',
      count: hovers,
      conversionRate: views > 0 ? (hovers / views) * 100 : 0,
    },
    {
      name: 'Time on Page',
      count: dwells,
      conversionRate: hovers > 0 ? (dwells / hovers) * 100 : 0,
    },
    {
      name: 'CTA Clicks',
      count: ctaClicks,
      conversionRate: dwells > 0 ? (ctaClicks / dwells) * 100 : 0,
    },
  ];
...
export async function getRealTimeProofUpdates(
  ...
  const proof = await _generateProofFromEvents(storyId, 'commuter', 5);
